...
OM_YM_peaks_up_pro<-OM_YM_peaks_up[grepl(x=OM_YM_peaks_up$annotation,pattern ='Promoter'),] 
up_gene <- data.frame('gene'=unique(OM_YM_peaks_up_pro$GeneName))
gene_last <- bitr(geneID = up_gene$gene, 
                  fromType = "SYMBOL",
                  toType = c("ENTREZID", "GENENAME"),
                  OrgDb = 'org.Mm.eg.db')

write.csv(up_gene,'./data_out/OM_YM/OM_YM_up_gene.csv')
...
write.csv(ego_up,'./data_out/OM_YM/OM_YM_ego_up.csv')
...
write.csv(ego_down,'./data_out/OM_YM/ego_down.csv')
...
ggsave('./plot/OM_YM/enrich_GO_OM_YM_WT.pdf', p, width = 12, height = 9, dpi = 300)
