...
save(sampleCount,file=paste("peak_data_collection", sep=""))
...
libsizes
write.csv(libsizes,'./data_out/OF_YF_libsizes.csv')
...
# export all diff
write.csv(peakAnno_all_df,file = paste('./data_out/FC-1_P-0.05/diffbind_peaks_anno_diff_OF_YF.csv') ,row.names = F)

# plot
peakAnno_df$annotation[grepl(x=peakAnno_df$annotation,pattern = 'Promoter')] <-'Promoter'

pdf(file = './plot/FC-1_P-0.05/OF_YF_all_open_bar.pdf',height = 3,width = 12)
plotAnnoBar(peakAnno_all_plot)
dev.off()

pdf(file = './plot/FC-1_P-0.05/OF_YF_all_open_pie.pdf',height = 6,width = 7)
...
# output
write.csv(peakAnno_up_df, './data_out/FC-1_P-0.05/OF_YF_sig_up.csv', row.names = FALSE)

# plot
peakAnno_up_df$annotation[grepl(x=peakAnno_up_df$annotation,pattern = 'Promoter')] <-'Promoter'

pdf(file = './plot/FC-1_P-0.05/OF_YF_up_open_bar.pdf',height = 3,width = 12)
plotAnnoBar(peakAnno_up_plot)
dev.off()

pdf(file = './plot/FC-1_P-0.05/OF_YF_up_open_pie.pdf',height = 6,width = 7)
...
# output
write.csv(peakAnno_down_df, './data_out/FC-1_P-0.05/OF_YF_sig_down.csv', row.names = FALSE)

# plot
peakAnno_down_df$annotation[grepl(x=peakAnno_down_df$annotation,pattern = 'Promoter')] <-'Promoter'

pdf(file = './plot/FC-1_P-0.05/OF_YF_down_open_bar.pdf',height = 3,width = 12)
plotAnnoBar(peakAnno_down_plot)
dev.off()

pdf(file = './plot/FC-1_P-0.05/OF_YF_down_open_pie.pdf',height = 6,width = 7)
