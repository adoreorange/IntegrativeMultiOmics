...
data1 <- read.csv("./data/YF-WT_vs_OF-WT-ATAC_RRBS-quadrants.csv", row.names = 1)

data <- data1


# 3. set quadrant DEGs
data$log2FC_RRBS <- data$log2FC_RRBS
data_significant <- data %>%
  filter(FDR_RRBS < 0.01 & FDR_ATAC < 0.01)

quadrant1_genes <- data_significant %>%
  filter(log2FC_RRBS > 10 & log2FC_ATAC > 1) %>%
  pull(GeneSymbol)
...
write.csv(data[(data$quadrant %in% c("Q1_UpUp","Q3_DownDown")), ],'./anaData/YF-WT_vs_OF-WT-ATAC_RRBS-quadrants_37.csv')
...
df <- data
#####
ggplot(df, aes(x = log2FC_RRBS, y = log2FC_ATAC))+
  ...
  geom_point(data = df[!(df$quadrant %in% "ns"), ], aes(x=log2FC_RRBS, y=log2FC_ATAC, 
  ...
  xlab("RRBS_OF_WT_vs_YF_WT(log2FC)")+
  ylab("ATAC_OF_WT_vs_YF_WT(log2FC)") +
...
# plot
ggsave('./0530/OF_WT_vs_YF_WT_OM_WT_vs_YM_WT.png', height = 8, width = 8)
...
ggplot(df, aes(x = log2FC_RRBS, y = log2FC_ATAC))+
  ...
  geom_point(data = df[!(df$quadrant %in% "ns"), ], aes(x=log2FC_RRBS, y=log2FC_ATAC, 
                                                        fill = quadrant, colour = quadrant, 
                                                        size = -log10(FDR_ATAC)), shape = 21, stroke = 0.7) +
  scale_size_continuous(range = c(1, 3)) +
  scale_alpha_continuous(range = c(0.2, 0.85), limits = c(1, 3)) +
  scale_fill_manual(values = c("#E64A35", "#4DBBD4", "#6BD66B", "#F29F80")) +
  scale_color_manual(values = c("#E64A35", "#4DBBD4", "#6BD66B", "#F29F80")) +
  geom_text_repel(
    ...
    aes(x=log2FC_RRBS, y=log2FC_ATAC, label = GeneSymbol),
    color = "black",
    size = 4.5,
  ...
  xlab("RRBS_B(DNA methylation, YF_WT-OF_WT)")+
  ylab("ATAC_OF_WT_vs_YF_WT(log2FC)") +
  theme_bw(base_size = 15) +
  theme(legend.position = "none", 
        panel.grid.major = element_blank(), 
  ...
  geom_hline(yintercept = c(-1, 1), color = "grey80", linewidth = 0.3, linetype = "dashed")
# plot
ggsave('./plot/YF-WT_vs_OF-WT-ATAC_RRBS-quadrants.pdf', height = 8, width = 8)
