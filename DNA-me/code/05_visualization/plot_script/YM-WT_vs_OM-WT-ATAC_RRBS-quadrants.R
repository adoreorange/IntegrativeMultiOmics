# 可视化YM-WT和OM-WT的ATAC-seq和RRBS数据的四象限DEGs
...
data2 <- read.csv("./data/YM-WT_vs_OM-WT-ATAC_RRBS-quadrants.csv", row.names = 1)
...
write.csv(data[(data$quadrant %in% c("Q1_UpUp","Q3_DownDown")), ],'./anaData/YM-WT_vs_OM-WT-ATAC_RRBS-quadrants_37.csv')
...
ggplot(df, aes(x = log2FC_RRBS, y = log2FC_ATAC))+
  ...
  ylab("ATAC_OM_WT_vs_YM_WT(log2FC)") +
...
ggplot(df, aes(x = log2FC_RRBS, y = log2FC_ATAC))+
  ...
  geom_hline(yintercept = c(-1, 1), color = "grey80", linewidth = 0.3, linetype = "dashed")
# plot
ggsave('./plot/YM-WT_vs_OM-WT-ATAC_RRBS-quadrants.pdf', height = 8, width = 8)
