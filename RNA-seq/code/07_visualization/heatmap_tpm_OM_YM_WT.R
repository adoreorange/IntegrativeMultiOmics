...
OM <- data.frame(om=rev(grep('OM-WT',colnames(data),value = T)))
YM <- data.frame(ym=rev(grep('YM-WT',colnames(data),value = T)))
...
data.1 <- data.1[, match(c("YM-WT_REP2","YM-WT_REP1","OM-WT_REP2","OM-WT_REP1"), colnames(data.1))]
library(ComplexHeatmap)
...
col_split = c(rep('YM_WT',2),rep('OM_WT',2))
col_split =factor(col_split,levels = c('YM_WT','OM_WT'))
...
pdf('./plot_out/heatamp_tpm_OM_YM_WT.pdf',width = 8,height = 12)
...
Heatmap(data.1, col = col_fun,cluster_columns = T,cluster_rows = F,
        ...
        top_annotation = ha_top,column_split = col_split,row_title = NULL,
