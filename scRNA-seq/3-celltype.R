...
p = DotPlot(scRNA, features=unique(genes_to_check), assay = 'RNA') + 
  coord_flip() + #翻转
...
umap =DimPlot(scRNA, reduction = "umap",cols = my36colors,pt.size = 0.8,
                  group.by = "RNA_snn_res.0.1",label = T,label.box = F) +
  NoAxes() + 
...
celltype[celltype$ClusterID %in% c(4),2]='T&NK'
celltype[celltype$ClusterID %in% c(0,1,2,3),2]='B'
