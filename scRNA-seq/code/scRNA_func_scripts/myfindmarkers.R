myfindmarkers=function(seurat_object,filename,thresh.use=0.25,species="mouse",all.markers=NULL,min.pct=0.25,colors=colors2,
  ...
  if(is.null(all.markers)){
    all.markers <- FindAllMarkers(object = seurat_object, only.pos = TRUE, 
                                  thresh.use = thresh.use, min.pct = min.pct)
  }
  write.csv(all.markers,paste0(filename,"all.markers.csv"),row.names = F)
  top5 <- all.markers %>% group_by(cluster) %>% top_n(5, -p_val_adj)
  write.csv(top5,paste0(filename,"_clustertop05_gene.csv"),row.names = F)
  ...
  if(species=="mouse"){
    common_markers=c("Cd19","Ms4a1","Cd79a","Cd79b")
  }else{
    common_markers=c("CD19","MS4A1","CD79A","CD79B")
  }
  
  if(length(grep(pattern =  "^Ig[jkl]|^Igh[vdj]",rownames(seurat_object)))>0){
    antibody_name=rownames(seurat_object)[grep(pattern =  "^Ig[jkl]|^Igh[vdj]",rownames(seurat_object))]
