...
mymodule_score=function(seurat_object,genelist,cutoff=0.1,width.ppi=6,height.ppi=5,cutline=T,
  ...
  print(p3)
  dev.off()
  pdf(paste0(filename,"_score_cluster_umap.pdf"),width = width.ppi*1.3,height = height.ppi)
  p3=DimPlot(object = choose, reduction = 'umap',label = F,cols = colors_crc,group.by = "score_cluster",
             pt.size = pt.size)
  print(p3)
  dev.off()
  
  choose@assays$RNA@data=choose@assays$RNA@data[1:(nrow(choose)-1),]
  print(choose@meta.data$score_cluster)
  score = choose@meta.data
  write.csv(score,'./score.csv')
  #saveRDS(object = choose,"./choose.rds")
  return(score)
  return(choose)
}
